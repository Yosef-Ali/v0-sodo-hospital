...
interface CompanyActionsCardProps {
  companyId: string
  onEdit?: () => void
}

export function CompanyActionsCard({ companyId, onEdit }: CompanyActionsCardProps) {
  ...
  const handleDelete = async () => {
    ...
    if (result.success) {
      toast({
        ...
        description: "The company registration has been permanently removed.",
      })
      router.push("/company")
      router.refresh()
    } else {
      toast({
        title: "Delete Failed",
        description: result.error || "Could not delete company.",
        variant: "destructive",
      })
      setOpen(false) 
    }
          ...
          <AlertDialogContent className="bg-gray-800 border-gray-700 text-gray-200">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-gray-400">
                This action cannot be undone. This will permanently remove this company record and all associated documents.
              </AlertDialogDescription>
            </AlertDialogHeader>
